...
/**
 * An {@link ByteBuilderAllocator} that caches the buffers which are likely to be
 * reused during auto-expansion of the buffers.
 * <p>
 * In {@link SimpleByteBuilderAllocator}, the underlying {@link ByteBuffer} of the
 * {@link ByteBuilder} is reallocated on its capacity change, which means the newly
 ...
 * processors, {@link CachedByteBuilderAllocator} outperformed
 * {@link SimpleByteBuilderAllocator} in the following situation:
 * <ul>
 ...
 * environment.
 * <p>
 * {@link CachedByteBuilderAllocator} uses {@link ThreadLocal} to store the cached
 * buffer, allocates buffers whose capacity is power of 2 only and provides
 * performance advantage if {@link ByteBuilder#free()} is called properly.
 * 
 * @author <a href="http://mina.apache.org">Apache MINA Project</a>
 */
public class CachedByteBuilderAllocator implements ByteBuilderAllocator {
     ...
     */
    public CachedByteBuilderAllocator() {
     ...
     */
    public CachedByteBuilderAllocator(int maxPoolSize, int maxCachedBufferSize) {
        if (maxPoolSize < 0) {
    ...
    @Override
    public ByteBuilder allocate(int requestedCapacity, boolean direct) {
        int actualCapacity = ByteBuilder.normalizeCapacity(requestedCapacity);
        ByteBuilder buf;
    ...
    @Override
    public ByteBuilder wrap(ByteBuffer nioBuffer) {
    ...
    private class CachedBuffer extends AbstractByteBuilder {
        private final Thread ownerThread;

        private ByteBuffer buf;

        protected CachedBuffer(ByteBuffer buf) {
            super(CachedByteBuilderAllocator.this, buf.capacity());
        ...
        @Override
        protected ByteBuilder duplicate0() {
            return new CachedBuffer(this, buf().duplicate());
        }

        @Override
        protected ByteBuilder slice0() {
            return new CachedBuffer(this, buf().slice());
        }

        @Override
        protected ByteBuilder asReadOnlyBuffer0() {
